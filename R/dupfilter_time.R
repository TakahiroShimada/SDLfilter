...
#' @seealso \code{\link{dupfilter}}, \code{\link{dupfilter_exact}}, \code{\link{dupfilter_qi}}, \code{\link{dupfilter_space}}
...
dupfilter_time<-function (sdata, step.time=0) {
  ...
  sdata2<-dup.timing(sdata=sdata, step.time=step.time)
  sdata3<-dup.timing(sdata=sdata2, step.time=step.time)
  while(!(nrow(sdata2) %in% nrow(sdata3))) 
  {
    sdata3<-dup.timing(sdata=sdata2, step.time=step.time)
    sdata2<-dup.timing(sdata=sdata3, step.time=step.time)
  }
